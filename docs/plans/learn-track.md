...
follow. **Both are walkable ground at T4** (Ligar since 2026-09-23): every one of
their nine benches is a built instrument standing on the ground, worked with the
quest frame as a closeable 2D overlay. At T3 and below both are the pages they
always were, grading identically.
...
| 02 | Ligar | Basalt arches | Molecules | 4 (4 built, walkable at T4) |
...
- [x] **Ligar as walkable ground** (`three/ligar.js`, `world-data/ligar.json`,
  `materials/ligar-textures.js`): a basalt quarry at dusk with three arches, a
  columnar-basalt cut you walk down into, four conveyors out of it, and the four
  benches — Arch Terrace, Tube Forge (in the cut), Batch House and Weighbridge
  (on the east deck under the gantry). Layout generated by
  `tools/gen-ligar-layout.py`.
- [x] **The join bench built** (`engine/joinbench3d.js`), a subclass of the drawn
  bench so its tools are the same code. With it, all four Ligar quests are in
  `BUILT_BENCHES`; the quest modules changed by one import line each.
- [x] `tools/verify-ligar.mjs` (including a flood-fill walk test against the real
  colliders and a buried-body check), and `verify:bench` extended to Ligar's four
  instruments plus every join-bench stage through the real pointer path.

Deliberately **not** done, and why:

- The other eight worlds have no ground. Tallow was built end-to-end first and Ligar
  followed its pattern; what broke on the way (see CLAUDE.md, "Ligar") informs the rest —
  the same order `3d-conversion-prompts.md` §6 sets out for Erebus.
- ~~Ligar has no ground~~ — superseded 2026-09-23: Ligar is walkable at T4 and all
  four of its benches are built.
