 ...
 * end up doing. Bench three counted molecules because there were few enough to
...
/** How many particles (atoms, molecules or formula units) a hopper holds. The floor only ever reports this for a sample it could tip. */
export function countOf(hopper) {
  return (hopper.bins || []).reduce((n, b) => n + b.n, 0);
}

/**
 * What a hopper's particles are called, counted `n`. A sample with a CAT code
 * is one element, so it holds atoms; a compound declares `particle` —
 * 'molecule', or 'formula unit' for an ionic one — because a mole is a count of
 * whatever the substance is made of, and the readout has to say which.
 */
export function particleWord(hopper, n = 2) {
  const one = hopper?.particle || 'atom';
...
export const STAGES = [
  /* ---------------------------------------------------------------- 1 */
  {
    ...
    briefing: {
      speaker: SPEAKER,
      body: 'This bench tips a sample down a chute into bins by weight and counts what lands in each. "Weigh Sample" puts a whole sample on a balance, even one too full to tip.'
    },
    prompt: 'Sample B is too full to tip. Say how many atoms it holds.',
    controls: ['pour', 'balance'],
    hoppers: [
      { id: 'h1', label: 'SAMPLE A', code: 'CAT 06', note: '40 atoms, kept as the counting standard', bins: [{ mass: 12, n: 40 }] },
    ...
    widget: { type: 'number', min: 0, max: 400, step: 50, label: 'Atoms in Sample B' },
    hints: [
      'Tap Sample A, press "Tip Sample", then press "Weigh Sample" on it, and weigh Sample B as well.',
      'Sample A is 40 atoms weighing 480 in all, and Sample B holds the same kind of atom, so each of its atoms weighs the same.',
      'Divide the total weight of Sample B by the weight of a single atom from Sample A.'
    ],
    check(state) {
      if (state.number === 0) {
      ...
      if (state.number === 50) {
        return { ok: false, msg: 'Weigh both: Sample B is far heavier than Sample A, and Sample A already holds 40.' };
      }
      if (state.number !== 300) {
        return { ok: false, msg: 'Work out what one atom weighs from Sample A, then divide Sample B\'s total by it.' };
      }
      return { ok: true };
    },
    reward: {
      log: 'Sample B: 3600 on the pan, 300 atoms.',
      title: 'Counting By Weighing',
      body: 'Nobody counted Sample B and nobody has to: once you know what one atom weighs, a total weight divides straight into a count. A small sample you CAN count hands you the weight of one atom, and from there the biggest crate in the yard is countable. This is the only way anybody has ever counted atoms.'
    }
  },
  ...
  /* ---------------------------------------------------------------- 2 */
  {
    ...
    prompt: 'Say what one molecule of Sample B weighs.',
    controls: ['pour', 'balance'],
    hoppers: [
      { id: 'h3', label: 'SAMPLE A', note: '40 molecules, one CAT 08 to two CAT 01', particle: 'molecule', bins: [{ mass: 18, n: 40 }] },
      { id: 'h4', label: 'SAMPLE B', note: 'a full crate, one CAT 06 to four CAT 01, gate shut', particle: 'molecule', bulk: true, bins: [{ mass: 16, n: 250 }] }
    ],
    reference: ['CAT 01 — 1.0', 'CAT 06 — 12.0', 'CAT 08 — 16.0'],
    widget: { type: 'number', min: 10, max: 24, step: 1, label: 'Weight of one molecule of Sample B' },
    hints: [
      'Tap Sample A, press "Tip Sample", and read the weight stencilled under the bin that caught everything.',
      'Sample A\'s bin caught at 18, and its recipe is one CAT 08 at 16.0 with two CAT 01 at 1.0 each — so a molecule weighs what its atoms weigh added up.',
      'Read Sample B\'s recipe off its label, then add one listed mass from the plate for every atom in it.'
    ],
    check(state) {
      if (state.number === 10) {
      ...
      if (state.number === 13) {
        return { ok: false, msg: 'That counts one CAT 01. A molecule of Sample B holds four of them.' };
      }
      if (state.number === 18) {
        return { ok: false, msg: 'That is Sample A. Sample B holds a different recipe, so work its own out.' };
      }
      if (state.number !== 16) {
        return { ok: false, msg: 'Add up one molecule of Sample B: one listed mass for every atom in its recipe.' };
      }
      return { ok: true };
    },
    reward: {
      ...
      title: 'Formula Mass',
      body: 'Sample A landed in the bin marked 18, which is exactly 16.0 + 1.0 + 1.0 — the listed masses of the atoms in one molecule of it, added up. Sample B comes to 12 + 4 = 16 the same way. Adding up the listed masses of every atom in one molecule or formula unit of a substance gives its formula mass, and there is nothing more to it than that.'
    }
  },
  ...
  /* ---------------------------------------------------------------- 3 */
  {
    ...
    prompt: 'Every one of these three samples holds sixty atoms. Say what their weights have in common.',
    controls: ['pour', 'balance'],
    hoppers: [
      { id: 'h5', label: 'SAMPLE A', code: 'CAT 01', note: '60 atoms, counted in', bins: [{ mass: 1, n: 60 }] },
      { id: 'h6', label: 'SAMPLE B', code: 'CAT 06', note: '60 atoms, counted in', bins: [{ mass: 12, n: 60 }] },
      { id: 'h7', label: 'SAMPLE C', code: 'CAT 08', note: '60 atoms, counted in', bins: [{ mass: 16, n: 60 }] }
    ],
    reference: ['CAT 01 — 1.0', 'CAT 06 — 12.0', 'CAT 08 — 16.0'],
    widget: {
      type: 'choice',
      ...
      options: [
        ...
        { id: 'nothing', label: 'Nothing — three different elements give three unrelated totals', note: 'No relation between them.' },
    ...
    hints: [
      ...
      'The three totals came to 60, 720 and 960, so divide each one by the 60 atoms it holds.',
      'Divide each of the three totals by 60 and set each result beside the listed mass of that element on the plate.'
    ],
    check(state) {
      if (!state.choice) {
      ...
      if (state.choice === 'nothing') {
        return { ok: false, msg: 'Divide each total by the 60 atoms it holds, and compare the results with the plate.' };
      }
      if (state.choice === 'sum') {
  ...
  /* ---------------------------------------------------------------- 4 */
  {
    ...
    hints: [
      'Tip at least two of the samples and put each total next to the sixty atoms it came from.',
      'Equal counts weigh in the ratio of the listed masses, so to get equal counts you weigh out the listed masses themselves.',
      'Set grams of CAT 08 over 12.0 grams of CAT 06 equal to CAT 08\'s listed mass over CAT 06\'s listed mass, and solve for the grams.'
    ],
    check(state) {
      if (state.number === 8) {
        return { ok: false, notYet: true, msg: 'Set the counter to the figure you worked out.' };
      ...
      if (state.number === 24) {
        return { ok: false, msg: 'Doubling works for more of the same element. These are two different elements, so go by their listed masses.' };
      }
      if (state.number !== 16) {
        return { ok: false, msg: 'Equal counts weigh in the ratio of the listed masses. Compare CAT 08\'s listed mass with CAT 06\'s.' };
      }
      return { ok: true };
    },
    reward: {
  ...
  /* ---------------------------------------------------------------- 5 */
  {
    ...
    prompt: 'Sample A is one CAT 11 to one CAT 17. Say how many grams of it you would weigh out to have one mole of its formula units.',
    controls: ['balance'],
    hoppers: [
      { id: 'h8', label: 'SAMPLE A', note: 'a full crate, one CAT 11 to one CAT 17, gate shut', particle: 'formula unit', bulk: true, bins: [{ mass: 58.5, n: 200 }] }
    ],
    ...
    hints: [
      'Work out the formula mass of Sample A first.',
      'One mole of an element is its listed mass in grams, and a formula unit of a compound weighs the listed masses in it added up.',
    ...
    check(state) {
      ...
      if (state.choice === '355' || state.choice === '230') {
        return { ok: false, msg: 'That is one of the two elements on its own. A formula unit of Sample A holds both of them.' };
      }
      if (state.choice === '1170') {
        return { ok: false, msg: 'That is two of each. A formula unit of Sample A holds one CAT 11 and one CAT 17.' };
      }
      return { ok: true };
    },
    reward: {
      ...
      title: 'Molar Mass',
      body: 'The rule does not stop at elements: add up the listed masses in one molecule or formula unit of a compound and that many grams of it is one mole of them. Here 23.0 + 35.5 = 58.5, so 58.5 grams holds 6.02 x 10^23 formula units of it. That figure, grams per mole, is called the molar mass, and every substance has one.'
    }
  },
  ...
  /* ---------------------------------------------------------------- 6 */
  {
    ...
    controls: ['balance'],
    hoppers: [
      { id: 'h9', label: 'SAMPLE A', note: 'one drum, one CAT 08 to two CAT 01, gate shut', particle: 'molecule', bulk: true, bins: [{ mass: 18, n: 4 }] }
    ],
    ...
    hints: [
      ...
      'Add up the listed masses in one molecule to get the grams in one mole, then divide the drum\'s reading by that.'
    ],
    check(state) {
      if (state.number === 0) {
        return { ok: false, notYet: true, msg: 'Set the counter to the number you worked out.' };
      ...
      if (state.number === 1) {
        return { ok: false, msg: 'One mole is what one molecule weighs, in grams. Compare that with what the drum reads.' };
      }
      if (state.number === 2) {
        return { ok: false, msg: 'Two moles would weigh twice one mole. Compare that with what the drum reads.' };
      }
      if (state.number !== 4) {
        return { ok: false, msg: 'Work out what one mole weighs from the recipe, then divide the drum\'s reading by it.' };
      }
      return { ok: true };
    },
    reward: {
      ...
      body: 'The molar mass is the bridge between the two: divide a mass in grams by it to get moles, multiply moles by it to get grams. 72 grams at 18 grams per mole is 4 moles, and those 4 moles hold four times 6.02 x 10^23 molecules. Every real measurement you can take is a mass and every recipe is a count, so this is the conversion that joins the two.'
    }
  },

  /* ---------------------------------------------------------------- 7 */
  {
    title: 'Moles of Each Element',
    prompt: 'You have two moles of Sample A. Say how many moles of CAT 01 that holds.',
    controls: ['balance'],
    hoppers: [
      { id: 'h10', label: 'SAMPLE A', note: 'two drums, one CAT 08 to two CAT 01, gate shut', particle: 'molecule', bulk: true, bins: [{ mass: 18, n: 2 }] }
    ],
    reference: ['CAT 01 — 1.0', 'CAT 08 — 16.0'],
    widget: { type: 'number', min: 0, max: 8, step: 1, label: 'Moles of CAT 01' },
    hints: [
      'Write down the recipe of one molecule first: one CAT 08 and two CAT 01.',
      'A ratio does not care what you count in, so two CAT 01 per molecule is also two moles of CAT 01 for every mole of molecules.',
      'Multiply the number of moles of the compound by the count of CAT 01 atoms in each molecule.'
    ],
    check(state) {
      if (state.number === 0) {
        return { ok: false, notYet: true, msg: 'Set the counter to the number you worked out.' };
      ...
      if (state.number === 2) {
        return { ok: false, msg: 'Two is the number of moles of molecules, and of the CAT 08 in them. Each molecule holds two CAT 01, not one.' };
      }
      if (state.number === 1) {
        return { ok: false, msg: 'One molecule already holds two CAT 01, so one mole of molecules holds two moles of them.' };
      }
      if (state.number !== 4) {
        return { ok: false, msg: 'Each molecule holds two CAT 01, and you have two moles of molecules.' };
      }
      return { ok: true };
    },
    reward: {
      ...
      body: 'The numbers in a formula are a ratio of atoms, and a ratio does not care what unit you count in: two CAT 01 per molecule is also two moles of CAT 01 per mole of molecules. So two moles of this compound hold four moles of CAT 01 and two moles of CAT 08. That is why the formula written for a single molecule is the same formula a yard uses to weigh out a shipment.'
    }
  },
  ...
  /* ---------------------------------------------------------------- 8 */
  {
    ...
    check(state) {
      ...
      if (state.choice === 'd2') {
        return { ok: false, msg: 'Sample B is the heaviest drum, but its atoms are the heaviest too. Divide its weight by its listed mass before comparing.' };
      }
      if (state.choice === 'd1') {
        return { ok: false, msg: 'Divide every drum\'s weight by its listed mass, and compare all three results.' };
      }
      return { ok: true };
    },
    reward: {
  ...
  sections: [
    {
      ...
      body: 'Nobody can count atoms, so everybody weighs them instead. One mole of a substance is its formula mass in grams, and it always holds the same number of atoms, molecules or formula units, whichever the substance is made of — 6.02 x 10^23 of them.'
    },
    {
  ...
  async function runTool(tool) {
    ...
    if (tool === 'balance') {
      ...
      renderReadout(null, {
        ...
        body: hopper.bulk
          ? `The pan reads ${fmt(totalOf(hopper))} in all. Nothing on this floor can tell you how many ${particleWord(hopper)} that is.`
          : `The pan reads ${fmt(totalOf(hopper))} in all, over the ${countOf(hopper)} ${particleWord(hopper, countOf(hopper))} the chute counted.`
      });
      return;
    }
    ...
    if (tool === 'pour') {
      ...
      renderReadout(null, {
        ...
        body: `${shot.total} ${particleWord(hopper, shot.total)} landed: ${shot.totals.map((n, i) => `${n} at weight ${hopper.bins[i].mass}`).join(', ')}. That is ${fmt(totalOf(hopper))} in all.`
      });
      busy = false;
    ...
    frame.setReadout(`
      <div class="lq-readout-card">
        ...
        <div class="lq-readout-hold">${hopper?.bulk
          ...
          : hit.poured
            ? `${hit.count} ${particleWord(hopper, hit.count)} landed in it.`
            : 'Empty. Nothing has been tipped yet.'}</div>
        <p class="lq-readout-line">A bin only catches ${esc(particleWord(hopper))} of the weight marked under it.</p>
      </div>
    `);
  }
