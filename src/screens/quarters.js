        ...
        ` : `
          <div style="display: flex; flex-direction: column; gap: 0.75rem;">
            ${Array.from({ length: Math.min(clearedCount, 20) }, (_, i) => {
              ...
              return `
                <div style="padding: 0.75rem 1rem; background: var(--plate-200); border: 1px solid var(--border-durasteel); border-left: 2px solid var(--accent-green);">
    ...
    api.getTeamRoster(session.teamId).then(roster => {
      ...
      slot.innerHTML = `
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 0.75rem;">
          ${roster.map(m => `
            <div style="background: var(--plate-200); border: 1px solid var(--border-durasteel); padding: 0.75rem 1rem;">
