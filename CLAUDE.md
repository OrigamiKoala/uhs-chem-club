...
- T4 In-World Terminals: In T4, 2D full-page screens are integrated into the 3D world as `.in-world-terminal` tactical HUD overlays with `[X] FREE WALK` dismiss controls. Star Map holo-table integrates Sector 01 status, transmission, and disembarking; Quarters integrates crew profile and avatar customizer; Cargo Hold integrates cargo manifest and trinket locker; Comms integrates fleet chatter and leaderboards; Settings integrates graphics tier (T4 opt-in) and audio sliders.
