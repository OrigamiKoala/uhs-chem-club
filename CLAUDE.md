...
- T4 World Quest Deployment: In T4, `#/quest` enters the 3D Erebus world (`stage.enterWorldScene()`). The chemistry chamber deploys as an in-world instrument only when the player interacts with an active pylon, lighting its amber indicator in 3D upon clearance and returning cleanly to 3D terrain walking.
- First-person controls: `src/three/fps-controls.js` provides unconstrained WASD + sprint (Shift) + mouse look navigation with sliding physics collision against walls/obstacles, terrain height clamping on Erebus, and contextual `[E]` interaction prompts at ship terminals and pylons. Movement is gated behind active session authentication.
